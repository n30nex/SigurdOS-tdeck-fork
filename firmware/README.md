...
| `SigurdOS-tdeck-launcher.bin` | Same bytes as merged — feed to [bmorcelli/Launcher](https://github.com/bmorcelli/Launcher) for install as a Launcher app. Published as a [release asset](https://github.com/hermes-gadget/SigurdOS-tdeck/releases/latest); local builds emit it in `webflasher/` |
...
| **Web flasher** (e.g. flasher.sigurdos.dev) | `sigurdos-tdeck-full.bin` from generated `webflasher/` output or release assets | auto |
...
The merge preserves the ESP32-S3 bootloader flash mode with `flash_mode=keep`
so DIO boot headers are not rewritten. Flash frequency and size are read from
the board definition in `platformio.ini`.
...
| `webflasher/sigurdos-tdeck-full.bin` | Generated by local release builds; same as `sigurdos-tdeck-merged.bin` — for web flashers |
| `webflasher/sigurdos-tdeck-*.bin` | Generated individual components — for web-based or custom flashing |
| `webflasher/manifest.json` | Generated build manifest (version, hashes, offsets, metadata) |
...
from generated local output, or the matching `sigurdos-tdeck-full.bin` release asset.
